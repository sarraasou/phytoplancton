  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="annotation">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(annotation)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="annotationDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { annotation } = storeToRefs(useAnnotationStore());
const { getAnnotationById } = useAnnotationStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getAnnotationById(id);
  setCurrentPageBreadcrumbs("annotations", ["detail"]);
});
</script>
<style>
.bottom {
