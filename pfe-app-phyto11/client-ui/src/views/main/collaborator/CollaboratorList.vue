    ...
    >
      ...
      >
        <el-table-column :prop="'id'" :label="'id'" width="80">
          <template #default="scope">
            <el-tooltip placement="right-start">
              ...
              <router-link
                :data-test="`collaboratorList_${
                  collaboratorList[scope.$index].id
                }`"
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :data-test="`collaboratorList${fieldName}EditLink_${
                collaboratorList[scope.$index].id
              }`"
            ...
            <el-button
              :data-test="`collaboratorList${fieldName}DeleteBtn_${
                collaboratorList[scope.$index].id
              }`"
              @click="
                () => handleOpenConfirmModal(collaboratorList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === collaboratorList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(collaboratorList[scope.$index]?.id)
              "
            >
      ...
      <div class="footer__wrapper">
        <Pagination
          :itemsPerPage="collaboratorPagination.take"
          :set-items-per-page="setItemsPerPage"
          :page="
            Math.floor(
              collaboratorPagination.skip / collaboratorPagination.take
            ) + 1
          "
          :current-page-change="currentPageChange"
          :total="collaboratorPagination.total"
...
const entityPluralName = "collaborators";
const fieldsName = ref(["user", "project"]);
...
const {
  ...
  collaboratorExcelFile,
} = storeToRefs(useCollaboratorStore());
const {
  fetchCollaborators,
  deleteCollaborator,
  fetchDataExcelCollaborators,
  softDeleteCollaborator,
} = useCollaboratorStore();

async function dataToExcel() {
  await fetchDataExcelCollaborators();
  const workbook = XLSX.read(collaboratorExcelFile.value.toString());
  XLSX.writeFile(workbook, "collaboratorData.xlsx");
}
const deleteLoading = ref<boolean>(false);
const handleRemoveEntity = async (id: string) => {
  deleteLoading.value = true;
  await softDeleteCollaborator(id);
...
const currentPageChange = async (val: number) => {
  await fetchCollaborators({
    skip: (val - 1) * collaboratorPagination.value.take,
    take: collaboratorPagination.value.take,
  });
};
onMounted(async () => {
