  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="collaborator">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(collaborator)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="collaboratorDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { collaborator } = storeToRefs(useCollaboratorStore());
const { getCollaboratorById } = useCollaboratorStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getCollaboratorById(id);
  setCurrentPageBreadcrumbs("collaborators", ["detail"]);
});
</script>
<style>
.bottom {
