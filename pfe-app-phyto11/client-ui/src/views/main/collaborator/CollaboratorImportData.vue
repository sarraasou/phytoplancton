    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`collaboratorImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="collaboratorImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="collaboratorImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useCollaboratorStore());
const { createManyCollaborator } = useCollaboratorStore();
const router = useRouter();
import { useAuthStore } from "@/store/useAuth";
const { currentUser } = storeToRefs(useAuthStore());

const initialState = { userId: "", projectId: "" };
...
const collaboratorInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      collaboratorInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      collaboratorInsertData.value = collaboratorInsertData.value.map(
        (item: any) => {
          let res: any = {};
          for (let key in rest) {
      ...
      await createManyCollaborator(collaboratorInsertData.value);
