    ...
    <el-form :model="form" :rules="rules" ref="ruleFormRef">
      ...
      <div class="form_items">
        ...
        >
          <el-select
            v-model="form[field]"
            :data-test="`projectImportData_${field}`"
          >
            <el-option
      ...
      <div class="d-flex justify-content-end">
        <el-button
          data-test="projectImportDataFormCancelButton"
          @click="resetForm(ruleFormRef)"
          >Clear</el-button
        >
        <el-button
          data-test="projectImportDataFormSubmitButton"
          type="primary"
...
const { error } = storeToRefs(useProjectStore());
const { createManyProject } = useProjectStore();
const router = useRouter();
import { useAuthStore } from "@/store/useAuth";
const { currentUser } = storeToRefs(useAuthStore());

const initialState = { title: "", description: "", tool: "", userId: "" };
...
const projectInsertData: any = ref([]);
...
const handelUploadFile: UploadProps["onChange"] = (rawFile: any) => {
  if (rawFile) {
    ...
    reader.onload = (e) => {
      ...
      projectInsertData.value = jsonData;
    };
...
const onSubmit = (formEl: InstanceType<typeof ElForm> | undefined) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (valid && !isEqual(initialState, rest)) {
      projectInsertData.value = projectInsertData.value.map((item: any) => {
        let res: any = {};
        for (let key in rest) {
      ...
      await createManyProject(projectInsertData.value);
