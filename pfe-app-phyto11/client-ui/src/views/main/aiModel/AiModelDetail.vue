  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="aimodel">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(aimodel)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="aimodelDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { aimodel } = storeToRefs(useAiModelStore());
const { getAiModelById } = useAiModelStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getAiModelById(id);
  setCurrentPageBreadcrumbs("aimodels", ["detail"]);
});
</script>
<style>
.bottom {
