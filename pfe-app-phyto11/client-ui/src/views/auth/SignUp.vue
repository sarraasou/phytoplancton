<template>
  ...
  >
    ...
    <div class="d-flex bg-login">
      ...
      <div
        class="bgi-no-repeat bgi-position-x-center bgi-size bgi-position-center min-h-150px min-h-xl-300px mb-xl-10 test3 square2"
        style="
      ...
      <div
        class="bgi-no-repeat bgi-position-x-center bgi-size bgi-position-center min-h-250px min-h-xl-250px mb-xl-10 test3 w-25"
        style="
...
const signUp = async () => {
  try {
    ...
    // ✅ Cas principal : EN_ATTENTE
    if (result?.status === 'EN_ATTENTE') {
      await ElMessageBox.alert(
        `<div style="text-align:center;padding:10px">
          <div style="font-size:50px;margin-bottom:12px">⏳</div>
          <h3 style="color:#0077b6;margin-bottom:8px">
            Inscription réussie !
          </h3>
          <p style="color:#495057;margin-bottom:8px">
            Votre compte est <b>en attente de validation</b> 
    ...
    // ⚠️ fallback (au cas où backend change)
    if (result?.success === true) {
      Components.ElMessage.success("Inscription réussie")
      router.push({ name: 'sign-in' })
    }

  } catch (error: any) {
    Components.ElMessage.error(
      error?.message || failMessage || "Erreur lors de l'inscription"
    )
  } finally {
