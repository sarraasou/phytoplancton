 ...
 * @param {UploadUserFile[]} files List of files from el-upload
 * @returns {Promise<string[]>} List of uploaded image URLs
 */
export const uploadImagesToCloudinary = async (files: any[]): Promise<string[]> => {
  ...
  for (const file of files) {
    ...
    formData.append("file", file.raw); // .raw est fourni par el-upload
    formData.append("upload_preset", "uploadAPI"); // à créer dans Cloudinary
    formData.append("cloud_name", "dsiekn5sr"); // remplace par ton cloud name

    const { data } = await axios.post(
      "https://api.cloudinary.com/v1_1/dsiekn5sr/image/upload",
      formData
    );
