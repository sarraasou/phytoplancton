...
export const useAppConfigStore = defineStore("appconfig-store", {
  ...
  actions: {
    async fetchAppConfigs(payload?: IPagination) {
      try {
        const { data } = await service.nest.appConfigControllerFindMany({
    ...
    async fetchDataExcelAppConfigs() {
      try {
        const {
          data,
        } = await service.nest.appConfigControllerFindDataForExcel();
    ...
    async softDeleteAppConfig(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.appConfigControllerUpdate(payload, {
          deletedAt: new Date(),
        });
        this.error = null;
    ...
    async deleteAppConfig(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.appConfigControllerDelete(payload);
    ...
    async editAppConfig(payload: { id: string; data?: AppConfigUpdateInput }) {
      ...
      try {
        ...
        const { data } = await service.nest.appConfigControllerUpdate(
          payload.id,
          editAppConfigData
        );
      ...
      async editManyAppConfig(payload: { data: AppConfigUpdateInput; where: any }) {
        this.isLoading  = true;
        try {
          const { data } = await service.nest.appConfigControllerUpdateMany(
            payload.data,
            payload.where
    ...
    async getAppConfigById(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.appConfigControllerFindOne(payload);
        this.appconfig = {
          ...data,
        };
        this.error = null;
    ...
    async createAppConfig(payload?: { data: AppConfigCreateInput }) {
      ...
      try {
        ...
        const { data } = await service.nest.appConfigControllerCreate(
          createAppConfigData
        );
    ...
    async createManyAppConfig(payload: any) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.appConfigControllerCreateMany(
          payload
        );
        this.error = null;
      } catch (err: any) {
