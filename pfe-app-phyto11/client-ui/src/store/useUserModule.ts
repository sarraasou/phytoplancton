...
export const useUserStore = defineStore("user-store", {
  ...
  actions: {
    ...
    async fetchDataExcelUsers() {
      try {
        const { data } = await service.nest.userControllerFindDataForExcel();
    ...
    async softDeleteUser(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.userControllerUpdate(payload, {
          deletedAt: new Date(),
        });
        this.error = null;
        this.fetchUsers({
    ...
    async deleteUser(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.userControllerDelete(payload);
    ...
    async editUser(payload: { id: string; data?: UserUpdateInput }) {
      ...
      try {
        ...
        const { data } = await service.nest.userControllerUpdate(
          payload.id,
          editUserData
        );
      ...
      async editManyUser(payload: { data: UserUpdateInput; where: any }) {
        this.isLoading  = true;
        try {
          const { data } = await service.nest.userControllerUpdateMany(
            payload.data,
            payload.where
    ...
    async getUserById(payload: string) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.userControllerFindOne(payload);
        this.user = {
          ...data,
        };
        this.error = null;
    ...
    async createUser(payload?: { data: UserCreateInput }) {
      ...
      try {
        ...
        const { data } = await service.nest.authControllerInviteUserByEmail(
          userCreateData
        );
    ...
    async createManyUser(payload: any) {
      this.isLoading = true;
      try {
        const { data } = await service.nest.userControllerCreateMany(payload);
        this.error = null;
      } catch (err: any) {
        this.error = err.error;
      } finally {
